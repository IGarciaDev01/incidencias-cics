...
import { index, show } from '@/routes/panel/capital_humano/incidencias';
...
export default function Index({
    ...
    function handleLimpiarFechas() {
        router.get(
            index.url(),
            { ...filtros, fecha_inicio: undefined, fecha_fin: undefined },
            { preserveState: true, replace: true },
        );
    }
    ...
    return (
        <>
            <Head title="Incidencias — Capital Humano" />

            <div className="space-y-5 p-4 md:p-6">
                <div>
                    <h2 className="text-xl font-semibold text-gray-900">
                        Incidencias — Capital Humano
                    </h2>
                ...
                <div className="flex flex-wrap gap-3">
                    <form onSubmit={handleBuscar} className="flex gap-2">
                        <Input
                            ...
                            placeholder="Folio, empleado o nombre..."
                            className="w-64"
                        />
                    ...
                    <Select
                        value={filtros.area_id || '_all_'}
                        onValueChange={(v) =>
                            handleFiltro('area_id', v === '_all_' ? '' : v)
                        }
                    >
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                handleFiltro('fecha_inicio', e.target.value)
                            }
                        />
                    </div>
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3">
                                                <p className="font-medium text-gray-900">
                                                    {inc.reportante_nombre}
                                                </p>
                                                <p className="text-xs text-gray-500">
                                                    #{inc.numero_empleado} ·{' '}
                                                    {inc.tipo_solicitante}
                                                </p>
                                            </td>
                                            ...
                                            <td className="px-4 py-3">
                                                <span
                                                    className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${ESTADO_COLORS[inc.estado] ?? 'bg-gray-100 text-gray-800'}`}
                                                >
                                                    {ESTADO_LABELS[
                                                        inc.estado
                                                    ] ?? inc.estado}
                ...
                {incidencias.last_page > 1 && (
                    <div className="flex items-center justify-between text-sm">
                        <p className="text-gray-500">
                            Mostrando {incidencias.from}–{incidencias.to} de{' '}
                            {incidencias.total}
                        </p>
